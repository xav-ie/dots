# Flake check that each host's home-manager git settings use canonical casing, no collisions.
{ config, lib, ... }:
let
  # Map system types to their flake configuration attrsets
  systemConfigs = {
    darwin = config.flake.darwinConfigurations;
    linux = config.flake.nixosConfigurations;
  };
  ...
  checkGitSettings =
    gitSettings:
    let
      canonical = import ./_canonical-case-check.nix {
        inherit lib gitSettings;
      };
      collision = import ./_case-collision-check.nix {
        inherit lib gitSettings;
      };
    in
    {
