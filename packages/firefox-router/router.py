...
def main():
    argv = sys.argv[1:]
    # `firefox-router --profile-dir <Name>` prints the absolute directory of a
    # named profile (resolved from the Profile Groups DB) and exits. Exposed so
    # helpers (e.g. the just slack-tokens recipe) reuse this resolution instead
    # of reimplementing the profiles.ini + Profile Groups sqlite walk.
    if argv and argv[0] == "--profile-dir":
        d = profile_dir(ff_root(), argv[1]) if len(argv) > 1 else None
        if not d:
            sys.exit(1)
        print(d)
        return
    urls = [u for u in (to_open_arg(a) for a in argv) if u]
    ff = firefox_bin()
    root = ff_root()
    if not urls:
