...
def col-widths [pairs: list<any>] {
  let wa = (
    $pairs
    | each {|c| $c.0 | into string | str length }
    | append 1
    | math max
  )
  let wb = (
    $pairs
    | each {|c| $c.1 | into string | str length }
    | append 1
    | math max
  )
  [$wa, $wb]
